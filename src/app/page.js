...
export default async function Home() {
  ...
  // Generación masiva de 30 testimonios persuasivos (V9 Ultimate)
  const reviews = [
    // Reviews con Foto (Asegurate que review-1.jpg a review-6.jpg estén en public)
    ...
    // Reviews de Texto Puro (Persuasivos/Diversos)
    ...
    { name: "Priya R.", text: "TSA compliant for travel, but TSA did give it a good look during carry-on. Heats fast. Crema is superb." },
    ...
    { name: "Javier L.", text: "Fast shipping to Miami. Packaged securely. Instruction manual is concise. Modular Cleaning protocol works as described." },
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white selection:text-black antialiased pb-20 lg:pb-0">
      ...
      {/* CSS Mejorado para Cinta Infinita y Masonry columns */}
      ...
      <div className="bg-white text-black py-2 overflow-hidden relative z-50 border-b border-gray-300">
        <div className="animate-marquee whitespace-nowrap text-[10px] font-black tracking-[0.2em] uppercase flex items-center">
          {[...Array(15)].map((_, i) => (
            <span key={i} className="mx-8">GLOBAL LAUNCH: 50% OFF + FREE WORLDWIDE EXPRESS SHIPPING</span>
          ))}
        </div>
      </div>

      {/* Nav Profesional Minimalista */}
      <nav className="p-6 border-b border-white/5 flex justify-between items-center bg-[#050505]/95 backdrop-blur-md sticky top-0 z-40">
        <h1 className="text-xs font-bold tracking-[0.4em] uppercase text-white/50">DILEMMA DRIFT / GLOBAL OPERATIVE</h1>
        <a href="#buy-box" className="text-xs font-bold tracking-[0.2em] uppercase text-white bg-white/5 px-6 py-3 border border-white/10 hover:bg-white hover:text-black transition-colors">
            SECURE UNIT
        </a>
      </nav>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 pt-12 pb-24 flex flex-col lg:flex-row gap-12 lg:gap-20">
        ...
        <div className="w-full lg:w-[55%] flex flex-col gap-16 order-2 lg:order-1">
          ...
          {/* RESTAURACIÓN TÉCNICA (Tech Grid Specs) */}
          ...
          {/* Videos de Acción (Renombrados/Acomodados) */}
          ...
          <div>
            ...
            <div className="space-y-4 bg-[#0a0a0a] border border-white/10 p-8">
              ...
              {[ { f: "Portable Off-Grid", u: true, t: false }, { f: "Self-Heating Core", u: true, t: false }, { f: "18-Bar Extraction", u: true, t: true }, { f: "Multi-Capsule Compatibility", u: true, t: false }, { f: "TSA Compliant Carry-on", u: true, t: false }, { f: "2-Year Industrial Warranty", u: true, t: false } ].map(row => (
          ...
          {/* LA MURALLA DE CONFIANZA (Reviews Masonry Grid - 30 items) */}
          <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/10 pb-6 mb-12 gap-6">
               ...
               <div className="flex items-center gap-4 bg-[#0a0a0a] border border-white/5 px-6 py-4">
                  <div className="text-center">
                      ...
                      <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Industrial Rating</p>
                  </div>
                  <div className="flex flex-col gap-1">
                      <div className="flex text-yellow-500 gap-1">
                        {[...Array(5)].map((_, j) => <Star key={j} className="w-4 h-4 fill-current" />)}
                      </div>
                      <span className="text-xs font-bold text-gray-200">681+ VERIFIED BUYERS</span>
                  </div>
               </div>
            </div>
            
            {/* Masonry Columns Layout (V9 - Avalancha visual) */}
                ...
                <div key={i} className="bg-[#0a0a0a] border border-white/10 break-inside-avoid overflow-hidden flex flex-col hover:border-white/20 transition-colors">
                  {review.img && (
                    <div className="w-full aspect-[4/3] bg-[#111] relative border-b border-white/5">
                      <img src={review.img} alt={`Review asset ${i+1}`} className="absolute inset-0 w-full h-full object-cover opacity-90" />
                    </div>
                  )}
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="flex items-center gap-3 mt-auto pt-4 border-t border-white/5">
                       ...
                       <div className="flex flex-col">
                          ...
                          <p className="text-[9px] text-gray-500 uppercase tracking-widest">Verified Deployment Report</p>
                       </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="mb-12">
            ...
            <div className="space-y-4">
              {[ { q: "Do I need hot water or power to use it?", a: "Negative. The Dilemma Drift engine features a 2500mAh self-heating core. Just add cold water, double click to activate the thermal core, and it heats it to 90°C in under 200 seconds. It operates completely off-grid." }, { q: "Which capsules are compatible?", a: "Absolute modular versatility. It includes adapters for standard small capsules (Nespresso Original style), large pods (Dolce Gusto size), and a dedicated chamber for your own freshly ground coffee beans. Barista-level crema anywhere." }, { q: "Is it TSA compliant for flights?", a: "Yes, it is cleared for carry-on luggage. The internal boiling system is thermodynamically safe. However, airline regulations prohibit using the self-heating function *during* the flight. Perfect for use in the terminal or upon landing." }, { q: "How do I clean the modular chamber?", a: "Hassle-free operation. Simply fill the reservoir with fresh water and run a cycle without inserting ammunition (capsules). The industrial-grade 18-bar pressure system purges and cleans itself automatically in under 30 seconds." } ].map(faq => (
        ...
        {/* Right Column: Sticky Buy Box (ID added) */}
        <div id="buy-box" className="w-full lg:w-[45%] order-1 lg:order-2">
          <div className="sticky top-28 bg-[#080808] border border-white/10 p-8 shadow-2xl z-30">
            ...
            <div className="flex items-center gap-2 mb-4 text-yellow-500">
              ...
              <span className="text-gray-400 text-xs tracking-widest ml-2 uppercase font-bold">450+ Reports Deployed</span>
            </div>
